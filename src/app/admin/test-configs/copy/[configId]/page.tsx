...
import { getTestConfigById, addTestConfig } from "@/lib/test-config-service";
...
export default function CopyTestConfigPage() {
  ...
  const [sourceConfigName, setSourceConfigName] = useState('');
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (configData) {
          setSourceConfigName(configData.name);
          form.reset({
            ...configData,
            name: `Copy of ${configData.name}`,
            slug: '',
            published: false,
          });
        } else {
          toast({ title: "Error", description: "Source Test Configuration not found.", variant: "destructive" });
  ...
  async function onSubmit(data: TestConfigFormValues) {
    ...
    try {
      ...
      toast({ title: "Success", description: "Test configuration has been copied." });
      router.push("/admin/test-configs");
      router.refresh();
    } catch (error) {
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          <Card>
            <CardHeader><CardTitle>Copy Test Configuration</CardTitle><CardDescription>Create a new configuration based on "{sourceConfigName}".</CardDescription></CardHeader>
            <CardContent className="space-y-6">
              <FormField control={form.control} name="name" render={({ field }) => (<FormItem><FormLabel>New Configuration Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>)} />
              <FormField control={form.control} name="slug" render={({ field }) => (<FormItem><FormLabel>URL Slug (Optional)</FormLabel><FormControl><Input {...field} value={field.value || ''} /></FormControl><FormDescription>If left blank, a slug will be generated from the name.</FormDescription><FormMessage /></FormItem>)} />
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button type="submit" disabled={isSubmitting || loading}>{isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save Copy</Button>
          </div>
        </form>
      </Form>
    </div>
